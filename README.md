...
* 상품 추천 기준 
  - 바우만 타입 매칭: FIRST/SECOND/THIRD/FOURTH 각 항목이 다른 경우 제외하고 점수 부여
  - 리뷰 수에 따라 가중치 부여
...
* 리뷰 추천 기준
  - total_score =
    (사용자 ↔ 리뷰 작성자 바우만 점수 + 사용자 ↔ 상품 바우만 점수 + 리뷰 ↔ 상품 평점 유사도 점수 ) × (관리자 검증이면 1.2)
...
➕ 개선사항
1. 신제품 추천
2. 기존 16가지의 값만 가지는 바우만 타입을 더욱 세분화하여 퍼센테이지로 구분 및 상품에 대한 추천도 이와 더불어 더욱 정밀하게 제공(바우만 타입의 문자열로 매칭된 10진수화 ex. D83S10W92T53의 경우 DSWT이지만 DNWT와도 유사)
3. 상품의 성분을 기반으로 바우만 타입을 추천하는 로직 또는 AI를 통해 관리자의 상품 등록 시 편의성 증대
4. 상품 추천 봇을 통해 상품을 추천하고 즉각적인 피드백
5. 리뷰를 영상 매체와 같은 더욱 많은 콘텐츠로 접할 수 있도록 수정